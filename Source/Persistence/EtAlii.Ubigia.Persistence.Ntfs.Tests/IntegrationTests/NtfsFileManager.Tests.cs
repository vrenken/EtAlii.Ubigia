...
namespace EtAlii.Ubigia.Persistence.Ntfs.Tests
{
    ...
    public class NtfsFileManagerTests : NtfsStorageUnitTestContext
    {
        ...
        [Fact, Trait("Category", TestAssembly.Category)]
        public async Task NtfsFileManager_LoadFromFile_String()
        {
            // Arrange.
            var containerId = StorageTestHelper.CreateSimpleContainerIdentifier();
            var file = Storage.PathBuilder.GetFileName(Guid.NewGuid().ToString(), containerId);
        ...
        [Fact, Trait("Category", TestAssembly.Category)]
        public async Task NtfsFileManager_LoadFromFile_Ulong()
        {
            // Arrange.
            var containerId = StorageTestHelper.CreateSimpleContainerIdentifier();
            var file = Storage.PathBuilder.GetFileName(Guid.NewGuid().ToString(), containerId);
        ...
        [Fact, Trait("Category", TestAssembly.Category)]
        public async Task NtfsFileManager_LoadFromFile_Ulong_LongFilename()
        {
            // Arrange.
            var containerId = StorageTestHelper.CreateLongContainerIndentifier(Storage.ContainerProvider);
            var file = Storage.PathBuilder.GetFileName(Guid.NewGuid().ToString(), containerId);
