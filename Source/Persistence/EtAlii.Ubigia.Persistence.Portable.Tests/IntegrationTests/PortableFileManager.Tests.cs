...
namespace EtAlii.Ubigia.Persistence.Portable.Tests
{
    ...
    public class PortableFileManagerTests : PortableStorageUnitTestContext
    {
        ...
        [Fact, Trait("Category", TestAssembly.Category)]
        public async Task PortableFileManager_LoadFromFile_String()
        {
            // Arrange.
            var containerId = StorageTestHelper.CreateSimpleContainerIdentifier();
            var file = Storage.PathBuilder.GetFileName(Guid.NewGuid().ToString(), containerId);
        ...
        [Fact, Trait("Category", TestAssembly.Category)]
        public async Task PortableFileManager_LoadFromFile_Ulong()
        {
            // Arrange.
            var containerId = StorageTestHelper.CreateSimpleContainerIdentifier();
            var file = Storage.PathBuilder.GetFileName(Guid.NewGuid().ToString(), containerId);
        ...
        [Fact, Trait("Category", TestAssembly.Category)]
        public async Task PortableFileManager_LoadFromFile_Ulong_LongFilename()
        {
            // Arrange.
            var containerId = StorageTestHelper.CreateLongContainerIndentifier(Storage.ContainerProvider);
            var file = Storage.PathBuilder.GetFileName(Guid.NewGuid().ToString(), containerId);
