...
namespace EtAlii.xTechnology.Threading
{
    ...
    /// </summary>
    public class ContextCorrelator : IContextCorrelator
    {
        // Source: https://github.com/serilog/serilog/issues/1015
        private static ImmutableDictionary<string, string> Items
        {
            get => _internalItems.Value ?? (_internalItems.Value = ImmutableDictionary<string, string>.Empty);
            set => _internalItems.Value = value;
        }
        private static readonly AsyncLocal<ImmutableDictionary<string, string>> _internalItems = new();
        ...
        /// <returns></returns>
        public IDisposable BeginCorrelationScope(string key, string value, bool throwWhenAlreadyCorrelated = true)
        {
