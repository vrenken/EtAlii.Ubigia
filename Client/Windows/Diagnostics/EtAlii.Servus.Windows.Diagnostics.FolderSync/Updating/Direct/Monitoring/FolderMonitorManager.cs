namespace EtAlii.Servus.Diagnostics.FolderSync
{
    ...
    internal class FolderMonitorManager : BindableBase
    {
        ...
        public void Start()
        {
            try
            {
                _logger.Info("Starting FolderSync");
                IsRunning = true;
                foreach (var monitor in _monitors)
                {
                    StartMonitor(monitor);
                }
                _logger.Info("Started FolderSync");
            }
            catch (Exception e)
            {
                HasManagerError = true;
                _logger.Critical("Fatal exception starting FolderSync", e);
                _logger.Info("Restarting FolderSync");
                Task.Delay(2000);
        ...
        public void Stop()
        {
            try
            {
                _logger.Info("Stopping FolderSync");
                IsRunning = false;
                foreach (var monitor in _monitors)
                {
                    StopMonitor(monitor);
                ...
                _logger.Info("Stopped FolderSync");
            }
            catch (Exception e)
            {
                HasManagerError = true;
                _logger.Critical("Fatal exception stopping FolderSync", e);
                Task.Delay(2000);
                HasManagerError = false;
            }
        }
