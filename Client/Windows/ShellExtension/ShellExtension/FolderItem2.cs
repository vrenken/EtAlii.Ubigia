...
namespace EtAlii.Servus.Client.Windows.ShellExtension
{
	...
	public class FolderItem2 : NSEFolder
	{
        ...
        public FolderItem2(string fullPath)
		{
            FullPath = fullPath;
		...
		public override void GetDataObject(GetDataObjectEventArgs e)
		{
			...
			{
				if (e.Children[i] is FolderItem2)
				{
					var m = e.Children[i] as FolderItem2;
                    files[i] = m.FullPath;
				}
				else
				{
					var c = e.Children[i] as FileItem2;
					files[i] = c.fullPath;
				}
			}
		...
		public override bool OnDelete(ExecuteMenuitemsEventArgs e)
		{
			...
			{
				if (item is FileItem2)
				{
					var c = item as FileItem2;
				...
				{
                    FolderItem2 c = item as FolderItem2;
		...
		static FolderItem2()
		{
            ...
            colName.FormatIdentifier = new Guid("3B52BD2B-6B2B-47A2-A500-9F2CBD7C7F93");
			colName.PropertyIdentifier = 10;

			ShellColumn colSize = new ShellColumn("Size");
            colSize.FormatIdentifier = new Guid("9A422E1B-6BF4-4A83-B052-1E95CFBAB429");
			colSize.PropertyIdentifier = 12;

			ShellColumn colType = new ShellColumn("Type");
            colType.FormatIdentifier = new Guid("E9C1286B-E6CD-4F98-AA3B-3B5809DFEB06");
        ...
        */
		...
		public override NSEItem DeserializeChild(BinaryReader reader)
		{
			...
			if (folder==0)
			{
				return new FileItem2(temp);
			}
				// else return the folder item if the folder exists
			else if (folder==1)
			{
				return new FolderItem2(temp);
			}
		...
		public override int CompareTo(NSEItem item2, ShellColumn column)
		{
			...
			if (item2 is FileItem2)
				return -1;

			var dir2 = item2 as FolderItem2;
		...
		public override bool OnOpen(ExecuteMenuitemsEventArgs e)
		{
			...
			{
				var fi = e.Children[0] as FileItem2;
				if (fi != null)
				{
