namespace EtAlii.Servus.Api.Logical.Tests
{
    ...
    public class LogicalTestContext : ILogicalTestContext
    {
        ...
        public async Task AddTime(ILogicalContext context, int year, int month, int day, int hour, int minute, int second)
        {
            var scope = new ExecutionScope(false);

            var timeRoot = await context.Roots.Get("Time");

            var yearEntry = await context.Nodes.Add(timeRoot.Identifier, year.ToString("D4"), scope);
            var monthEntry = (IEditableEntry)await context.Nodes.Add(yearEntry.Id, month.ToString("D2"), scope);
            var dayEntry = (IEditableEntry)await context.Nodes.Add(monthEntry.Id, day.ToString("D2"), scope);
            var hourEntry = (IEditableEntry)await context.Nodes.Add(dayEntry.Id, hour.ToString("D2"), scope);
            var minuteEntry = (IEditableEntry)await context.Nodes.Add(hourEntry.Id, minute.ToString("D2"), scope);
            var secondEntry = (IEditableEntry)await context.Nodes.Add(minuteEntry.Id, second.ToString("D2"), scope);
        }
        public async Task AddTime(ILogicalContext context, int year, int month, int day, int hour, int minute)
        {
            var scope = new ExecutionScope(false);
        ...
        public async Task AddTime(ILogicalContext context, int year, int month, int day, int hour)
        {
            var scope = new ExecutionScope(false);
        ...
        public async Task AddTime(ILogicalContext context, int year, int month, int day)
        {
            var scope = new ExecutionScope(false);
        ...
        public async Task AddTime(ILogicalContext context, int year, int month)
        {
            var scope = new ExecutionScope(false);
        ...
        public async Task AddTime(ILogicalContext context, int year)
        {
            var scope = new ExecutionScope(false);
