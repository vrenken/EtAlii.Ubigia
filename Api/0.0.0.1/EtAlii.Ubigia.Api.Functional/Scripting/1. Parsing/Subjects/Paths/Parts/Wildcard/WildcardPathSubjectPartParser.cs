    ...
    internal class WildcardPathSubjectPartParser : IWildcardPathSubjectPartParser
    {
            ...
            INodeFinder nodeFinder)
        {
            ...
            var beforeTextParser = new LpsParser("Before", true,
                (Lp.One(c => _constantHelper.IsValidConstantCharacter(c)).OneOrMore().Id(BeforeTextId)) |
                (Lp.One(c => c == '\"') + Lp.One(c => _constantHelper.IsValidQuotedConstantCharacter(c, '\"')).OneOrMore().Id(BeforeTextId) + Lp.One(c => c == '\"')) |
                (Lp.One(c => c == '\'') + Lp.One(c => _constantHelper.IsValidQuotedConstantCharacter(c, '\'')).OneOrMore().Id(BeforeTextId) + Lp.One(c => c == '\''))
            ).Maybe();

            var afterTextParser = new LpsParser("After", true,
                (Lp.One(c => _constantHelper.IsValidConstantCharacter(c)).OneOrMore().Id(AfterTextId)) |
                (Lp.One(c => c == '\"') + Lp.One(c => _constantHelper.IsValidQuotedConstantCharacter(c, '\"')).OneOrMore().Id(AfterTextId) + Lp.One(c => c == '\"')) |
                (Lp.One(c => c == '\'') + Lp.One(c => _constantHelper.IsValidQuotedConstantCharacter(c, '\'')).OneOrMore().Id(AfterTextId) + Lp.One(c => c == '\''))
            ).Maybe();
