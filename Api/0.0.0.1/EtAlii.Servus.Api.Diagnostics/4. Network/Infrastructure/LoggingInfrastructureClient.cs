...
//        public async Task<T> Get<T>(string address, ICredentials credentials = null)
//        {
//            var message = String.Format("Invoking GET on infrastructure (Url: {0} Type: {1})", address, typeof(T).Name);
//            _logger.Info(message);
//            var start = Environment.TickCount;

//            var result = await _client.Get<T>(address, credentials);
...
//        public async Task<T> Post<T>(string address, T value = default(T), ICredentials credentials = null) 
//            where T : class
//        {
...
//            var result = await _client.Post<T>(address, value, credentials);
...
//        public async Task<U> Post<T, U>(string address, T value = default(T), ICredentials credentials = null) 
//            where T : class 
//            where U : class
//        {
...
//            var result = await _client.Post<T, U>(address, value, credentials);
