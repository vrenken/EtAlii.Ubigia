namespace EtAlii.Servus.Api.Transport.WebApi
{
    ...
    internal class WebApiEntryDataClient : WebApiClientBase, IEntryDataClient<IWebApiSpaceTransport>
    {
        ...
        {
            var address = Connection.AddressFactory.Create(Connection.Storage, RelativeUri.Entry, UriParameter.SpaceId, Connection.Space.Id.ToString());
        ...
        {
            var changeAddress = Connection.AddressFactory.Create(Connection.Storage, RelativeUri.Entry);
        ...
        public async Task<IReadOnlyEntry> Get(Identifier entryIdentifier, ExecutionScope scope, EntryRelation entryRelations = EntryRelation.None)
        {
            return await scope.Cache.GetEntry(entryIdentifier, async () =>
            {
                ...
                var address = Connection.AddressFactory.Create(Connection.Storage, RelativeUri.Entry, UriParameter.EntryId,
            ...
            ExecutionScope scope, EntryRelation entryRelations = EntryRelation.None)
        {
            ...
            {
                var address = Connection.AddressFactory.Create(Connection.Storage, RelativeUri.RelatedEntries,
