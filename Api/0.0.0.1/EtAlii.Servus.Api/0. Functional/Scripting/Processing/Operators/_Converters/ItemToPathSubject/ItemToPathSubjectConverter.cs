    ...
    internal class ItemToPathSubjectConverter : IItemToPathSubjectConverter
    {
        ...
        public ItemToPathSubjectConverter()
        {
            _converterSelector = new Selector<object, Func<object, PathSubject>>()
                ...
                .Register(item => item is INode, item => new PathSubject(new ConstantPathSubjectPart(((INode)item).Type))) // << ????
                .Register(item => item is string, item => new PathSubject(new ConstantPathSubjectPart((string) item)));
        }
