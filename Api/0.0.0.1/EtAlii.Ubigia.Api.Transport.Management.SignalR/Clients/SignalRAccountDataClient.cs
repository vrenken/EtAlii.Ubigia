namespace EtAlii.Ubigia.Api.Transport.Management.SignalR
{
    ...
    public class SignalRAccountDataClient : IAccountDataClient<ISignalRStorageTransport>
    {
        ...
        public async Task<Account> Add(string accountName, string accountPassword, AccountTemplate template)
        {
            ...
            return await _invoker.Invoke<Account>(_proxy, SignalRHub.Account, "Post", account, template.Name);
        }

        public async Task Remove(Guid accountId)
        {
            await _invoker.Invoke(_proxy, SignalRHub.Account, "Delete", accountId);
        }
        ...
        public async Task<Account> Change(Guid accountId, string accountName, string accountPassword)
        {
            ...
            return await _invoker.Invoke<Account>(_proxy, SignalRHub.Account, "Put", accountId, account);
        }

        public async Task<Account> Change(Account account)
        {
            return await _invoker.Invoke<Account>(_proxy, SignalRHub.Account, "Put", account.Id, account);
        }

        public async Task<Account> Get(string accountName)
        {
            return await _invoker.Invoke<Account>(_proxy, SignalRHub.Account, "GetByName", accountName);
        }

        public async Task<Account> Get(Guid accountId)
        {
            return await _invoker.Invoke<Account>(_proxy, SignalRHub.Account, "Get", accountId);
        }

        public async Task<IEnumerable<Account>> GetAll()
        {
            return await _invoker.Invoke<IEnumerable<Account>>(_proxy, SignalRHub.Account, "GetAll");
        }
        ...
        public async Task Connect(IStorageConnection<ISignalRStorageTransport> storageConnection)
        {
            await Task.Run(() =>
            {
                _proxy = storageConnection.Transport.HubConnection.CreateHubProxy(SignalRHub.Account);
            });
        }
